...
export default function CitizenLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      localStorage.setItem("citizenUser", JSON.stringify(user ?? data));
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo: `${redirectUrl}/citizen/report`,
        },
      });
            ...
            <motion.div whileHover={{ scale: 1.02 }}>
              <Button
                ...
                className={`w-full py-3 font-semibold text-white rounded-lg shadow-md ${
                  loading
                    ? "from-gray-400 to-gray-500 cursor-not-allowed bg-gradient-to-r"
